...
function loadDatabase(){
  ...
  try {
    const newDatabase = yaml.safeLoad(fs.readFileSync('theScoop.yml', 'utf8'), function(err){
      if (err) throw err;
    }).database;
    return newDatabase;
  } catch (e) {
...
function saveDatabase(){
  const yaml = require('js-yaml');
  const fs = require('fs');
  fs.writeFile('theScoop.yml', yaml.safeDump(JSON.parse(JSON.stringify(database)), function(err){
    if (err) throw err;
  }));
}
